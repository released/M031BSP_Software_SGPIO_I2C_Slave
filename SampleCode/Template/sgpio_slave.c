...
#define SGPIO_DATA_BITS_PER_SLOT          SGPIO_SLAVE_DATA_BITS_PER_SLOT
...
static SGPIO_FRAME_DECODED_CALLBACK_T g_sgpio_frame_callback = (SGPIO_FRAME_DECODED_CALLBACK_T)0;

/*_____ F U N C T I O N S __________________________________________________*/
void SGPIO_SetFrameDecodedCallback(SGPIO_FRAME_DECODED_CALLBACK_T callback)
{
    g_sgpio_frame_callback = callback;
}
...
void SGPIO_Process(void)
{
    ...
    if (g_sgpio_frame_callback != (SGPIO_FRAME_DECODED_CALLBACK_T)0)
    {
        g_sgpio_frame_callback(&local_frame);
    }
